...
{
    Book book(categoryID(), prodName_, price_, qty_, isbn_, author_);
    return &book;
}
...
{
    Clothing clothing(categoryID(), prodName_, price_, qty_, size_, brand_);
    return &clothing;
}
...
{
    Movie movie(categoryID(), prodName_, price_, qty_, genre_, rating_);
    return &movie;
}
